...
import { OrderListPersonApi, OrderZxExportApi } from '#/api/core/order';
  ...
  proxyConfig: {
    ...
    ajax: {
      query: async ({ page }, formValues) => {
        // 使用直投订单查询接口，type=2 代表API个人直投
        const result = await OrderListPersonApi(
          {
...
const exportEvent = async () => {
  ...
  try {
    ...
    const exportUrl = await OrderZxExportApi(
      {
