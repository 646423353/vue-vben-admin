...
const handleSuccess: UploadProps['onSuccess'] = (
  ...
  if (fileIndex !== -1 && fileItem) {
    ...
    // Default tag can be empty or 'otherPictureList'
    fileItem.tag = '';
...
watch(
  () => [props.id, props.files],
  ([newId, newFiles]) => {
    if (newId && Array.isArray(newFiles)) {
      // Avoid infinite loop if the update comes from our own emit
      const json = JSON.stringify(newFiles);
      if (lastEmitted.value === json) {
        return;
      }
      // Also check against current internal state to be double sure (e.g. if lastEmitted was cleared or mismatch)
      // Actually the lastEmitted check should be sufficient for the "Echo" problem.
      // But let's Update lastEmitted to `json` to prevent re-emit when we update fileList below?
      // No, we want fileList update to potentially trigger emit if it sanitizes data.
      // But if newFiles IS exactly what we want, we update fileList.
      // fileList update will trigger watcher. Watcher will calculate `data`.
      // If `data` stringifies to `json` (which equals `lastEmitted` if we set it here?), then we won't emit.

      // Strategy:
      // 1. Check against lastEmitted. If match, ignore.
      // 2. If no match (external update), we update fileList.
      //    This WILL trigger fileList watcher.
      //    We want that watcher to NOT emit if the resulting data matches what we just got.
      //    So we should set `lastEmitted = json` HERE too?
      //    If we set `lastEmitted = json`, then fileList watcher will see `json` matches `lastEmitted` and skip emit.
      //    This prevents the "Echo of API Load" being sent back to parent redundantly.
  ...
  <div class="w-full">
    ...
    >
      <div
        ...
        :key="file.uid"
        class="relative flex max-w-full flex-col overflow-hidden rounded-lg border border-gray-200 bg-white p-3 shadow-sm transition-shadow hover:shadow-md sm:p-4"
      >
        <div class="mb-3 flex items-start gap-2 sm:gap-3">
          <!-- Thumbnail -->
          <div
            class="h-16 w-16 flex-shrink-0 overflow-hidden rounded-md border border-gray-100 bg-gray-50 sm:h-20 sm:w-20"
          >
            <img
              v-if="file.url"
              :src="file.url"
          ...
          <div class="min-w-0 flex-1 overflow-hidden">
            <div
              class="truncate pr-6 text-sm font-medium text-gray-900"
              :title="file.name"
            >
              {{ file.name }}
            </div>
        ...
        <div class="mt-auto border-t border-gray-50 pt-2">
          <div class="flex flex-col gap-1 sm:flex-row sm:items-center sm:gap-2">
            ...
            <ElFormItem
              :prop="`${propPrefix || 'files'}.${index}.cateId`"
              :rules="{
                required: true,
                message: '请选择标签',
                trigger: 'change',
